...
<head>
    ...
    @php
        ...
        // Ambil site_title dulu, jika kosong ambil site_name, jika kosong baru default SERAYA VISUAL
        $browserTitle = $siteSetting->site_title ?? $siteSetting->site_name ?? 'SERAYA VISUAL';
    @endphp

    <!-- Title Dinamis Tab Browser Admin -->
    <title>{{ $browserTitle }} - Admin Panel</title>

    <!-- Dynamic Favicon Admin -->
        ...
        <aside class="w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700/80 hidden md:flex flex-col shadow-sm z-10 transition-colors duration-300">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                @if(auth()->check() && auth()->user()->role === 'admin')
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-all duration-200 {{ request()->routeIs('admin.dashboard') ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 shadow-sm border border-blue-100 dark:border-blue-800/40' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/50' }}">
                ...
                @else
                    ...
                    <!-- MENU HISTORY CHAT USER -->
                    <a href="{{ route('user.dashboard') }}#pesan-konsultasi" class="flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-all duration-200 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/50">
